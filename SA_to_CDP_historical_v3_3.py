...
# Energima imports
from SauterAPI_module_v3_3 import sautervision_login, sautervision_dataprocessing_hist, unix_to_ticks, local_sensorlist
import energima_logger
# Cognite imports
from CogniteAPI_module_sauter_v1_1 import upload_objects, upload_datapoints_historical, cdf_startup, get_last_timestamp
...
def process_datapoints(logger, sensor_id):
    """
    downloads datapoint from API, and sends it to uploader CDF, threads are locked in get request, and unlocked when
    ...
    if not sensor_data["HistoricalDataValues"] == []:
        upload_datapoints_historical(logger, sensor_id, sensor_data, cdf_startup.api_key, cdf_startup.project_name,
                                     cdf_startup.log)
